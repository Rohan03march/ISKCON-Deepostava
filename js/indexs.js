    ...
    try {
        ...
        if (docSnap.exists()) {
            ...
            readings.forEach((entry, index) => {
                createEntry(bookEntriesDiv, entry.name, entry.description, entry.timestamp, index);
...
function createEntry(container, name, description, timestamp, index) {
    ...
    entryDiv.innerHTML = `
        ...
        <span class="short-description">"${formattedDescription.substring(0, 100)}..."</span>
    ...
    deleteBtn.addEventListener('click', async function() {
        if (confirm('Are you sure you want to delete this entry?')) {
            try {
                ...
                if (entryToDelete) { // Ensure entry is defined
...
document.getElementById('submitBtn').addEventListener('click', async function() {
    ...
    if (bookName && bookDescription) {
        ...
        if (bookEntriesDiv.dataset.editing === 'true') {
            ...
            readingsArray[editingIndex] = { name: bookName, description: bookDescription, timestamp: new Date() };
            ...
            try {
                ...
                const currentEntryDiv = document.getElementById(`entry-${editingIndex}`);
                currentEntryDiv.remove(); // Remove the old entry from the DOM
                createEntry(bookEntriesDiv, bookName, bookDescription, new Date(), editingIndex); // Create the updated entry
            ...
            readingsArray.push({ name: bookName, description: bookDescription, timestamp: new Date() }); // Add to the array
            createEntry(bookEntriesDiv, bookName, bookDescription, new Date(), entryIndex); // Create a new entry
        ...
        bookInfoDiv.innerHTML = "Please select a book or enter a book name, and provide some information about it.";
    }
});
